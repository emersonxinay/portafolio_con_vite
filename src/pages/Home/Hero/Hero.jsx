...
import resume from '../../../assets/resume/emerson-espinoza.pdf';
import resumeEn from '../../../assets/resume/emerson-espinoza.pdf';
  ...
  return (
    <motion.div
      ...
      <div className='grid items-center gap-4 xl:max-w-2xl text-center xl:text-start'>
        {/* Hero Left side */}
        <h1 className='font-black text-2xl xxs:text-3xl xs:text-4xl sm:text-5xl xl:text-[3.3rem]'>
          {t('hero.title')}{' '}
        ...
        <div className='flex gap-4 relative m-auto xl:m-0'>
          <button
            ...
            className='font-semibold px-6 py-2 bg-gradient-to-r  from-sky-500 to-sky-700 ring-0 rounded-xl hover:scale-105 transition-transform flex items-center gap-2'>
            {t('hero.contact')}{' '}
            <TbChevronsDown size={'20px'}></TbChevronsDown>
          </button>
          <div className='font-semibold p-0.5 bg-gradient-to-r  from-sky-500 to-sky-700 ring-0 rounded-xl overflow-hidden w-fit hover:scale-105 transition-transform'>
            <a
              href={i18n.language == 'EN' ? resumeEn : resume}
              target='_blank'
              className='px-6 py-2 bg-[#112] rounded-xl flex items-center gap-2'>
              {t('hero.cv')} <AiFillEye></AiFillEye>
            </a>
          </div>
        </div>
      </div>
